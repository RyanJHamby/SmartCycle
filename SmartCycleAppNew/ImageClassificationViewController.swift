...
class ImageClassificationViewController: UIViewController {
    ...
    //            The coordinates are normalized to the dimensions of the processed image, with the origin at the image's lower-left corner.
       ...
       func drawDetectionsOnPreviewNew(detections: [VNRecognizedObjectObservation]) ->String {
        var aString = ""
            for detection in detections {
                
                aString = detection.labels.map({"\($0.identifier) confidence: \($0.confidence)"}).joined(separator: "\n")
        ...
        DispatchQueue.main.async {
            ...
            if classifications.isEmpty {
                ...
                let descriptions = topClassifications.map { classification in
                    ...
                    return self.drawDetectionsOnPreviewNew(detections: classifications)
                }
